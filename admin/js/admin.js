...
function uploadImage(obj, w, h, size) {
	...
	window.top.art.dialog(
	    {id:winid, iframe:url, title:'上传', opacity: 0.2, width:'450', height:'100', lock:true}, 
		function(){
...
function uploadFile(obj, type, size) {
	...
	var winid = 'win_'+obj;
	window.top.art.dialog(
	    {id:winid, iframe:url, title:'上传', opacity: 0.2, width:'470', height:'150', lock:true}, 
		function(){
		    var d = window.top.art.dialog({id:winid}).data.iframe;
			var filename = d.document.getElementById('filename').value;
			if (filename) {
...
function uploadFiles(obj, setting) {
	...
	var winid = 'win_'+obj;
	window.top.art.dialog(
	    {id:winid, iframe:url, title:'上传', opacity: 0.2, width:'500', height:'420', lock:true}, 
		function(){
		    var d = window.top.art.dialog({id:winid}).data.iframe;
