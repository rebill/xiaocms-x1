...
class CategoryController extends Admin {
	...
	public function addAction() {
	    if ($this->post('submit')) {
	        ...
	        if ($this->post('addall')) {
				...
				$html = '<script type="text/javascript">parent.document.getElementById(\'leftMain\').src =\' ?s=admin&c=content&a=category\';</script>';
				$this->show_message($this->getCacheCode('category') . $html. '添加成功', 1, url('admin/category/index'));
			}
	    }
    ...
    public function editAction() {
	    if ($this->post('submit')) {
	        ...
	        $data  = $this->post('data');
	        if (empty($data['catname'])) $this->show_message('请填写栏目名称');
	        if ($this->post('typeid')==1 && $this->category->check_catdir($catid, $data['catdir'])) $this->show_message( '栏目路径为空或者已经存在');
	        $data['typeid']  = $this->post('typeid');
	        $result = $this->category->set($catid, $data);
	        if (is_numeric($result)) {
				$html = '<script type="text/javascript">parent.document.getElementById(\'leftMain\').src =\' ?s=admin&c=content&a=category\';</script>';
	            $this->show_message($this->getCacheCode('category') .$html. $data['catname'].' 栏目修改成功', 1, url('admin/category/index'));
	        } else {
	...
	public function delAction() {
	    ...
	    if ($this->category->del($catid)) {
			$html = '<script type="text/javascript">parent.document.getElementById(\'leftMain\').src =\' ?s=admin&c=content&a=category\';</script>';
	        $this->show_message($this->getCacheCode('category') .$html. '删除成功', 1, url('admin/category/index'));
	    } else {
