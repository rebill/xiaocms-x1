...
class TemplateController extends Admin {
    ...
    public function editAction() {
		...
		$local = str_replace(XIAOCMS_PATH, '', $filepath);	
		if (!is_file($filepath)) $this->show_message($dir.$filename.'该文件不存在',2, '?s=admin&c=template&dir='.$dir);
		if ($this->isPostForm()) {
	...
	public function addAction() {
		...
		if ($this->isPostForm()) {
			...
			if (!in_array($ext, array('html', 'css', 'js'))) $this->show_message('文件名后缀不对',2, '?s=admin&c=template&dir='.$dir );
...
//		还未实现
	...
	public function cacheAction($show=0) {
	    ...
	    $list = $file_list->get_file_list(TEMPLATE_DIR);
