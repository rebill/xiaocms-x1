...
class IndexController extends Member {
    ...
    public function __construct() {
		...
		if (!$this->memberinfo['status']) $this->show_message('您还没有通过审核。',2,url('index/index/')); //判断审核
	...
	public function listAction() {
		...
		if ($mid && !isset($this->membermodel[$mid])) $this->show_message('会员模型不存在');
		$this->view->assign(array(
		    'page'    => $page,
			'modelid' => $mid,
		));
	...
	public function passwordAction() {
	    if ($this->isPostForm()) {
			...
			if ($data['password2'] != $data['password3']) $this->show_message('两次密码输入不一致。');
