...
class LoginController extends Member {
	...
	public function indexAction() {
	    if (!$this->isLogin(1)) $this->show_message('您不能重复登录', 2, url('member/index'));
	    if ($this->isPostForm()) {
		    $data   = $this->post('data');
			if ($this->site_config['MEMBER_LOGINCODE'] && !$this->checkCode($this->post('code'))) $this->show_message('验证码不正确');
			if (empty($data['username']) || empty($data['password'])) $this->show_message('用户名或密码不能为空');
			...
			if (empty($member)) $this->show_message('会员名不存在');
			if ($member['password'] != md5(md5($data['password']))) $this->show_message('密码错误');
