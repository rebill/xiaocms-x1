...
class RegisterController extends Member {
	...
	public function indexAction() {
	    ...
	    if (!$this->isLogin(1)) $this->show_message('您已经登录了，不能再次注册。', url('member/'));
	    if ($this->isPostForm()) {
		    $data = $this->post('data');
			if ($this->site_config['MEMBER_REGCODE'] && !$this->checkCode($this->post('code'))) $this->show_message('验证码不正确');
			$this->check($data);
			$uid  = $this->reg($data);
			if (empty($uid)) $this->show_message('注册失败');
	...
	private function check($data) {
	    ...
	    if (empty($data['username'])) $this->show_message('请填写会员名');
		if (!$this->is_username($data['username'])) $this->show_message('两次输入密码不一致');
		if (empty($data['password'])) $this->show_message('密码不能为空');
		if ($data['password'] != $data['password2']) $this->show_message('两次输入密码不一致');
		if (!is_email($data['email'])) $this->show_message('邮箱格式不正确');

		$member = $this->member->from(null, 'id')->where('email=?', $data['email'])->select(false);
		if ($member) $this->show_message('邮箱已经存在，请重新注册');
		$member = $this->member->from(null, 'id')->where('username=?', $data['username'])->select(false);
		if ($member) $this->show_message('会员已经存在');
	}
	...
	private function reg($data) {
		...
		if (!isset($this->membermodel[$data['modelid']])) $this->show_message('会员模型不存在');
