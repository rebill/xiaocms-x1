...
class ModelModel extends Model {
	...
	public function set($modelid=0, $data) {
		...
		if ($data['typeid'] == 1) {
			...
			$sql = "CREATE TABLE IF NOT EXISTS `" . $table . "` (`id` mediumint(8) NOT NULL AUTO_INCREMENT,`cid` mediumint(8) NOT NULL,`userid` mediumint(8) NOT NULL,`username` char(20) NOT NULL,`listorder` tinyint(3) unsigned NOT NULL DEFAULT '0',`status` tinyint(2) unsigned NOT NULL DEFAULT '1',`inputtime` int(10) unsigned NOT NULL DEFAULT '0', `ip` char(20) NULL,PRIMARY KEY (`id`),KEY `listorder` (`listorder`),KEY `status` (`status`),KEY `inputtime` (`inputtime`),KEY `userid` (`userid`),KEY `cid` (`cid`)) ENGINE=MyISAM DEFAULT CHARSET=utf8;";
			$this->query($sql);
		}
		//创建Model
